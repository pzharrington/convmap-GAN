...
class DCGAN:
    ...
    def build_discriminator(self):
        ...
        for lyrIdx in range(self.nlayers): 
            if lyrIdx==0:
                 # feed input dims 
                 dmodel.add(Conv2D(filters=self.nconvfilters[lyrIdx], kernel_size=self.convkern, strides=self.convstride, 
    ...
    def build_generator(self):
        gmodel = Sequential()
        gmodel.add(Dense(8*8*(2*self.ndeconvfilters[0]), input_shape=(1,self.noise_vect_len)))
        gmodel.add(BatchNormalization())
        gmodel.add(ReLU())
        gmodel.add(Reshape((2*self.ndeconvfilters[0], 8, 8)))
        for lyrIdx in range(self.nlayers):
            gmodel.add(Conv2DTranspose(self.ndeconvfilters[lyrIdx], self.convkern, strides=self.convstride, 
                                       padding='same', data_format=self.datafmt))
